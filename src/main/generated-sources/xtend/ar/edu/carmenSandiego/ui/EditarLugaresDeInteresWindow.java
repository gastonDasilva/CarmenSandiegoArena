...
public class EditarLugaresDeInteresWindow extends Dialog<CrearEditarPaisAppModel> {
  public EditarLugaresDeInteresWindow(final WindowOwner owner, final CrearEditarPaisAppModel model) {
    super(owner, model);
  ...
  protected void createFormPanel(final Panel mainPanel) {
    ...
    final Procedure1<List<LugarDeInteres>> _function = new Procedure1<List<LugarDeInteres>>() {
      public void apply(final List<LugarDeInteres> it) {
        ...
        ArenaXtendExtensions.operator_spaceship(_value, "lugarASacar");
      }
    };
    ObjectExtensions.<List<LugarDeInteres>>operator_doubleArrow(_list, _function);
    final NotNullObservable elementSelected1 = new NotNullObservable("lugarASacar");
    ...
    final Procedure1<Button> _function_1 = new Procedure1<Button>() {
      public void apply(final Button it) {
        ...
        final Action _function = new Action() {
          public void execute() {
            CrearEditarPaisAppModel _modelObject = EditarLugaresDeInteresWindow.this.getModelObject();
            _modelObject.eliminarLugarSeleccionado();
          }
        };
        it.onClick(_function);
        it.<Object, ControlBuilder>bindEnabled(elementSelected1);
      }
    };
    ObjectExtensions.<Button>operator_doubleArrow(_button, _function_1);
    Panel _panel_1 = new Panel(mainPanel);
    ColumnLayout _columnLayout = new ColumnLayout(2);
    final Panel selecPanel = _panel_1.setLayout(_columnLayout);
    Selector<LugarDeInteres> _selector = new Selector<LugarDeInteres>(selecPanel);
    final Procedure1<Selector<LugarDeInteres>> _function_2 = new Procedure1<Selector<LugarDeInteres>>() {
      public void apply(final Selector<LugarDeInteres> it) {
        ObservableItems<Selector<LugarDeInteres>, LugarDeInteres, ListBuilder<LugarDeInteres>> _items = it.items();
        Binding _spaceship = ArenaXtendExtensions.operator_spaceship(_items, "lugaresDeInteresPosibles");
        PropertyAdapter _propertyAdapter = new PropertyAdapter(LugarDeInteres.class, "nombreLugar");
        _spaceship.setAdapter(_propertyAdapter);
        ObservableValue<Control, ControlBuilder> _value = it.<ControlBuilder>value();
    ...
    ObjectExtensions.<Selector<LugarDeInteres>>operator_doubleArrow(_selector, _function_2);
